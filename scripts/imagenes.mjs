...
const FOTOS = path.join(RAIZ, "public", "fotos");

// Los anchos que piden de verdad los sitios donde se pintan estas fotos:
// las tarjetas de la cinta miden 200-320 px de CSS, así que con 240 y 400
// se cubren las pantallas de 1x y de 2x. El original (640) se queda para 3x.
const ANCHOS = [240, 400];
...
const archivos = (await fs.readdir(FOTOS)).filter(
  (n) => n.endsWith(".webp") && !esVariante(n)
);

let generadas = 0;
let saltadas = 0;
// Qué anchos acabó teniendo cada foto. Se escribe a un JSON que leen los
// componentes: si el srcset se armara a ciegas con la lista de ANCHOS, las
// fotos que ya eran pequeñas apuntarían a variantes que este script no llegó a
// crear, y el navegador se comería un 404 por cada una.
const manifiesto = {};

for (const nombre of archivos) {
  const origen = path.join(FOTOS, nombre);
  ...
  for (const ancho of ANCHOS) {
    ...
    // agrandar una foto no la mejora, solo pesa más.
    if (!meta.width || meta.width <= ancho) continue;

    const destino = path.join(FOTOS, `${base}@${ancho}w.webp`);
    if (!forzar) {
      try {
  ...
  manifiesto[`/fotos/${nombre}`] = anchosListos.sort((a, b) => a - b);
}
...
console.log(
  `imágenes: ${generadas} generadas, ${saltadas} ya estaban al día ` +
    `(${archivos.length} originales × ${ANCHOS.length} anchos)`
);
